...
public:
    ...
    struct bot_druid_ai : public bot_ai
    {
        ...
        bool MaintainTankHots(uint32 diff)
        {
            static constexpr float TANK_HOT_RANGE = 40.0f;
            static constexpr uint32 TANK_HOT_CHECK_INTERVAL = 750;
            ...
            // "someone is dying right now" cases.
            if (me->GetPowerType() != POWER_MANA || GetManaPCT(me) < 25)
                return false;

            auto isMaintainableTank = [this](Unit const* member) -> bool
                {
                    if (!member)
                        return false;
            ...
            auto isValidTankTarget = [&](Unit* member) -> bool
                {
                    if (!member || member == me || !member->IsAlive() || !member->IsInMap(me))
                        return false;
                    if (member->isPossessed() || member->IsCharmed())
                    ...
                    if (member->IsNPCBot() && member->ToCreature()->IsTempBot())
                        return false;
                    if (me->GetDistance(member) > TANK_HOT_RANGE)
                        return false;
                    if (!isMaintainableTank(member))
                        return false;

                    // This is combat maintenance, not idle buffing. Prefer actual tanks
                    // who are fighting or whose party is already fighting.
                    if (!member->IsInCombat() && member->getAttackers().empty() && !master->GetBotMgr()->IsPartyInCombat(false))
                        return false;

                    return true;
                };

            auto shouldUseRegrowthForTank = [&](Unit* tank) -> bool
                {
                    if (!tank)
                        return false;

                    if (GetHealthPCT(tank) <= 92)
                        return true;

                    // Keep Regrowth up more aggressively in dungeon/raid boss-style
                    // situations, where the tank is likely to keep taking damage.
                    if (tank->getAttackers().empty())
                        return false;

                    for (Unit const* attacker : tank->getAttackers())
                    {
                        ...
                        if (creature->GetMaxHealth() > me->GetMaxHealth() * 4)
                            return true;
                    }
            ...
            auto tryMaintainTankHots = [&](Unit* tank) -> bool
                {
                    if (!isValidTankTarget(tank))
                        return false;

                    // If the tank dips and already has a druid HoT from this bot,
                    // use Swiftmend before spending a GCD on more maintenance.
                    if (IsSpellReady(SWIFTMEND_1, diff, false) && GetHealthPCT(tank) <= 65 &&
                        tank->GetAuraEffect(SPELL_AURA_PERIODIC_HEAL, SPELLFAMILY_DRUID, 0x50, 0x0, 0x0, me->GetGUID()))
                    {
                        if (doCast(tank, GetSpell(SWIFTMEND_1)))
                            return true;
                    }

                    // Rejuvenation is the core rolling tank HoT. Keep this up first.
                    if (uint32 REJUVENATION = GetSpell(REJUVENATION_1))
                    {
                        if (IsSpellReady(REJUVENATION_1, diff) &&
                            !tank->GetAuraEffect(SPELL_AURA_PERIODIC_HEAL, SPELLFAMILY_DRUID, 0x10, 0x0, 0x0, me->GetGUID()))
                        {
                            if (doCast(tank, REJUVENATION))
                                return true;
                        }
                    }

                    // Lifebloom smooths active tank damage. Build to 3 stacks and
                    // refresh when it is close to blooming.
                    if (uint32 LIFEBLOOM = GetSpell(LIFEBLOOM_1))
                    {
                        if (IsSpellReady(LIFEBLOOM_1, diff) && (!tank->getAttackers().empty() || GetHealthPCT(tank) < 95))
                        {
                            AuraEffect const* bloom = tank->GetAuraEffect(SPELL_AURA_PERIODIC_HEAL, SPELLFAMILY_DRUID, 0x0, 0x10, 0x0, me->GetGUID());
                            if (!bloom || bloom->GetBase()->GetStackAmount() < 3 || bloom->GetBase()->GetDuration() < 3000)
                            {
                                if (doCast(tank, LIFEBLOOM))
                                    return true;
                            }
                        }
                    }

                    // Regrowth is stronger and more expensive, so keep it for real tanking
                    // pressure instead of mindlessly refreshing it on every trash nibble.
                    if (uint32 REGROWTH = GetSpell(REGROWTH_1))
                    {
                        if (IsSpellReady(REGROWTH_1, diff) && shouldUseRegrowthForTank(tank) &&
                            !tank->GetAuraEffect(SPELL_AURA_PERIODIC_HEAL, SPELLFAMILY_DRUID, 0x40, 0x0, 0x0, me->GetGUID()))
                        {
                            if (doCast(tank, REGROWTH))
                                return true;
                        }
                    }
            ...
            std::vector<Unit*> members = BotMgr::GetAllGroupMembers(master);
            for (Unit* member : members)
                if (tryMaintainTankHots(member))
                    return true;

            for (auto const& [_, bot] : *master->GetBotMgr()->GetBotMap())
                if (tryMaintainTankHots(bot))
                    return true;

            if (tryMaintainTankHots(master))
                return true;
