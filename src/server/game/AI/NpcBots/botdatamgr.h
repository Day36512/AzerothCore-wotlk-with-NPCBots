...
public:
    ...
    static bool GenerateRatedArenaOpponentBots(Player const* groupLeader, BattlegroundQueue* queue, PvPDifficultyEntry const* bracketEntry, GroupQueueInfo const* playerQueueInfo);
    static bool IsRatedArenaOpponentBot(Creature const* bot);
    static float GetRatedArenaOpponentDamageDoneMultiplier();
    static float GetRatedArenaOpponentDamageTakenMultiplier();
    static void SetRatedArenaOpponentLevelOverride(uint32 entry, uint8 level);
    static uint8 GetRatedArenaOpponentLevelOverride(uint32 entry);
    static void ClearRatedArenaOpponentLevelOverride(uint32 entry);
    static void SetRatedArenaOpponentGearItemLevelOverride(uint32 entry, uint32 minItemLevel, uint32 maxItemLevel);
    static std::pair<uint32, uint32> GetRatedArenaOpponentGearItemLevelOverride(uint32 entry);
    static void ClearRatedArenaOpponentGearItemLevelOverride(uint32 entry);
