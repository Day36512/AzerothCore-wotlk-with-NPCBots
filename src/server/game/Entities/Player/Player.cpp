...
void Player::_ApplyItemBonuses(ItemTemplate const* proto, uint8 slot, bool apply, bool only_level_scale /*= false*/)
{
    ...
    ScalingStatValuesEntry const* ssv = ScalingStatValue ? sScalingStatValuesStore.LookupEntry(ssd_level) : nullptr;    if (only_level_scale && !ssv)
        return;
    ...
    if (ssv)
    {
        if (uint32 ssvarmor = ssv->getArmorMod(ScalingStatValue))
            if (ScalingStatValue > 0 || ssvarmor < proto->Armor) // Check to avoid higher values than stat itself (heirloom OR items with correct armor value)
                armor = ssvarmor;
    }
