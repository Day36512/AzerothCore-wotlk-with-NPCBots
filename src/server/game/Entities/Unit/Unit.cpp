...
SpellCastResult Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
    if (!spellInfo)
    {
        LOG_DEBUG("entities.unit", "CastSpell: unknown spell by caster {}", GetGUID().ToString());
...
SpellCastResult Unit::CastSpell(Unit* victim, uint32 spellId, TriggerCastFlags triggerFlags /*= TRIGGER_NONE*/, Item* castItem /*= nullptr*/, AuraEffect const* triggeredByAura /*= nullptr*/, ObjectGuid originalCaster /*= ObjectGuid::Empty*/)
{
    ...
    if (!spellInfo)
    {
        LOG_DEBUG("entities.unit", "CastSpell: unknown spell {} by caster {}", spellId, GetGUID().ToString());
        return SPELL_FAILED_SPELL_UNAVAILABLE;
    }
...
void Unit::GetProcAurasTriggeredOnEvent(AuraApplicationProcContainer& aurasTriggeringProc, std::list<AuraApplication*>* procAuras, ProcEventInfo eventInfo)
{
    ...
    ObjectGuid const targetGuid = GetGUID();

    // Dinkle, hackfix: Snapshot first so nested proc/script work cannot invalidate the map iterator we are walking.
    std::list<AuraApplication*> auraSnapshot;
    if (procAuras)
    {
        auraSnapshot.assign(procAuras->begin(), procAuras->end());
    }
    else
    {
        for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr != GetAppliedAuras().end(); ++itr)
            auraSnapshot.push_back(itr->second);
    }
    ...
    auto processAuraApplication = [&](AuraApplication* aurApp)
        {
            ...
            // The AuraApplication must still be the active application owned by this Aura for this target.
            if (aura->GetApplicationOfTarget(targetGuid) != aurApp)
                return;
            ...
            uint8 procEffectMask = aura->GetProcEffectMask(aurApp, eventInfo, now);
            if (procEffectMask)
            {
                // Revalidate after script/proc checks, because they may alter aura state.
                if (aurApp->GetRemoveMode())
                    return;
                ...
                // PrepareProcToTrigger can also change aura state, so validate again before queuing.
                if (aurApp->GetRemoveMode())
                    return;
            ...
            else if (spellInfo->HasAttribute(SPELL_ATTR2_PROC_COOLDOWN_ON_FAILURE))
            {
                // Script checks may have removed or replaced the aura, so do not touch it again until revalidated.
                if (aurApp->GetRemoveMode())
                    return;
                ...
                if (SpellProcEntry const* procEntry = sSpellMgr->GetSpellProcEntry(aura->GetId()))
                    aura->AddProcCooldown(procEntry, now);
            }
        };

    for (std::list<AuraApplication*>::const_iterator itr = auraSnapshot.begin(); itr != auraSnapshot.end(); ++itr)
