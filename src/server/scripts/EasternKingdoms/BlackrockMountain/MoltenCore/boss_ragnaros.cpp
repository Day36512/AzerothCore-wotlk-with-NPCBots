...
enum Texts
{
    ...
    SAY_KNOCKBACK = 7, // "By fire be purged!"
    SAY_WRATH = 8,
    SAY_KILL = 9,
    SAY_MAGMABURST = 10,
};
...
struct boss_ragnaros : public BossAI
{
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 const eventId = events.ExecuteEvent())
        {
            ...
            {
                //Dinkle custom
                if (Unit* target = SelectRandomManaUser(100.0f))
                {
...
private:
    ...
    bool IsTankForTargeting(Unit* u) const
    {
        if (!u)
            return false;
        ...
        {
            static constexpr uint32 RIGHTEOUS_FURY = 25780;
            if (p->getClass() == CLASS_PALADIN && p->HasAura(RIGHTEOUS_FURY))
                return true;
        }
    ...
    Unit* SelectRandomManaUser(float range)
    {
        std::list<Unit*> nearby;
        Acore::AnyUnitInObjectRangeCheck check(me, range);
        Acore::UnitListSearcher<Acore::AnyUnitInObjectRangeCheck> searcher(me, nearby, check);
        ...
        nonTanks.reserve(pool.size());
        for (Unit* u : pool)
            if (!IsTankForTargeting(u))
                nonTanks.push_back(u);

        std::vector<Unit*>& pick = nonTanks.empty() ? nonTanks : nonTanks;
        if (pick.empty())
            pick = pool;

        return pick[urand(0u, uint32(pick.size() - 1))];
    }
    //Dinkle custom end
...
{
    SPELL_LAVA_BURST_A, SPELL_LAVA_BURST_B, SPELL_LAVA_BURST_C, SPELL_LAVA_BURST_D,
    SPELL_LAVA_BURST_E, SPELL_LAVA_BURST_F, SPELL_LAVA_BURST_G, SPELL_LAVA_BURST_H
};
...
{
    SPELL_SUMMON_SON_OF_FLAME_A, SPELL_SUMMON_SON_OF_FLAME_B, SPELL_SUMMON_SON_OF_FLAME_C, SPELL_SUMMON_SON_OF_FLAME_D,
    SPELL_SUMMON_SON_OF_FLAME_E, SPELL_SUMMON_SON_OF_FLAME_F, SPELL_SUMMON_SON_OF_FLAME_G, SPELL_SUMMON_SON_OF_FLAME_H
};
