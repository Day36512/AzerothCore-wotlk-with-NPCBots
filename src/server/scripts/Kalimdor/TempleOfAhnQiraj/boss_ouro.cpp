...
struct boss_ouro : public BossAI
{
    ...
    bool HasLivingPlayerOrNpcbotInRange(float radius) const
    {
        // Players
        ...
        // NPCBots (creatures flagged as bots)
        std::list<Creature*> nearby;
        Acore::AnyUnitInObjectRangeCheck ccheck(me, radius);
        Acore::CreatureListSearcher<Acore::AnyUnitInObjectRangeCheck> csearcher(me, nearby, ccheck);
        Cell::VisitObjects(me, csearcher, radius);
        ...
        for (Creature* c : nearby)
        {
            ...
            if (!c->IsNPCBot())
                continue;
            // Only count them if Ouro is hostile to them (normal case)
            if (!me->IsHostileTo(c))
                continue;
            return true;
        }
    ...
    void StartWipeCheck()
    {
        scheduler.CancelGroup(GROUP_WIPE_CHECK);
        scheduler.Schedule(2s, GROUP_WIPE_CHECK, [this](TaskContext context)
            {
                if (!HasLivingPlayerOrNpcbotInRange(120.f))
                {
                    EnterEvadeMode(EVADE_REASON_NO_HOSTILES);
                    return; // do not repeat after evading
                }
                context.Repeat(2s);
            });
    }
    ...
    void JustDied(Unit* killer) override
    {
        ...
        me->GetCreatureListWithEntryInGrid(npcList, 15957, 200.0f); 
...
// 26093 - Quake (Ouro): Filter out NPCBots from taking school damage
class spell_ouro_quake_filter : public SpellScript
{
    PrepareSpellScript(spell_ouro_quake_filter);
        ...
        targets.remove_if([](WorldObject* obj)
            {
                if (Unit* unit = obj->ToUnit())
                    return unit->IsNPCBot(); // remove all NPCBots from hit list
                return false;
            });
    }
    ...
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_ouro_quake_filter::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
    }
};
...
void AddSC_boss_ouro()
{
    ...
    RegisterSpellScript(spell_ouro_quake_filter);
}
