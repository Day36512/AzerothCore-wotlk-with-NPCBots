-- ITEM BUDGET NORMALIZER BEGIN
-- Generated by tools/item_budget_normalizer/normalize_items.py
-- Date: 2026-06-23T08:24:42
-- Config: C:\AzerothCore\Burning_ Legacy_Server\AzerothCore-wotlk-with-NPCBots\tools\item_budget_normalizer\item_budget_config.yml
-- Dry-run reviewed: no

CREATE TABLE IF NOT EXISTS item_template_backup_budget_normalizer_20260623_082441 AS
SELECT *
FROM item_template
WHERE entry IN (31820, 31282, 30369, 27784, 31291, 24089, 27491, 27903, 27913, 28226, 28252, 28263, 29288, 29329, 29992);

-- 31820 Blessed Signet Ring
UPDATE item_template
SET
  stat_value1 = 11,
  stat_value2 = 10,
  stat_value3 = 10,
  stat_value4 = 10
WHERE entry = 31820;

-- 31282 Shroud of Spiritual Purity
UPDATE item_template
SET
  stat_value1 = 35,
  stat_value2 = 38,
  stat_value3 = 43
WHERE entry = 31282;

-- 30369 Cleansed Fel Pauldrons
UPDATE item_template
SET
  stat_value1 = 81,
  stat_value2 = 21,
  stat_value3 = 25
WHERE entry = 30369;

-- 27784 Scintillating Coral Band
UPDATE item_template
SET
  stat_value1 = 12,
  stat_value2 = 13,
  stat_value3 = 14,
  stat_value4 = 18
WHERE entry = 27784;

-- 31291 Crystalforged War Axe
UPDATE item_template
SET
  stat_value1 = 60,
  stat_value2 = 32
WHERE entry = 31291;

-- 24089 Blazing Eternium Band
UPDATE item_template
SET
  stat_value1 = 15,
  stat_value2 = 23,
  stat_value3 = 9
WHERE entry = 24089;

-- 27491 Signet of Repose
UPDATE item_template
SET
  stat_value1 = 14,
  stat_value2 = 15,
  stat_value3 = 17,
  stat_value4 = 12
WHERE entry = 27491;

-- 27903 Sonic Spear
UPDATE item_template
SET
  stat_value1 = 28,
  stat_value2 = 25,
  stat_value3 = 19
WHERE entry = 27903;

-- 27913 Whispering Blade of Slaying
UPDATE item_template
SET
  stat_value1 = 18,
  stat_value2 = 13
WHERE entry = 27913;

-- 28226 Timeslicer
UPDATE item_template
SET
  stat_value1 = 15,
  stat_value2 = 18
WHERE entry = 28226;

-- 28252 Bloodfyre Robes of Annihilation
UPDATE item_template
SET
  stat_value1 = 32,
  stat_value2 = 32,
  stat_value3 = 64
WHERE entry = 28252;

-- 28263 Stellaris
UPDATE item_template
SET
  stat_value1 = 18,
  stat_value2 = 10
WHERE entry = 28263;

-- 29288 Violet Signet
UPDATE item_template
SET
  stat_value1 = 15,
  stat_value2 = 15,
  stat_value3 = 10,
  stat_value4 = 19
WHERE entry = 29288;

-- 29329 Terokk's Quill
UPDATE item_template
SET
  stat_value1 = 46,
  stat_value2 = 28
WHERE entry = 29329;

-- ITEM BUDGET NORMALIZER END
